...
function getSelectedIds() {
  return Array.from(bookingChecks)
    .filter(function(input) {
      ...
      return input.checked && card && !card.classList.contains('hidden');
    })
    .map(function(input) {
      return input.value;
    });
...
function updateBulkState() {
  ...
  bookingChecks.forEach(function(input) {
    ...
    if (card) {
      card.classList.toggle('selected-booking', input.checked);
    }
  });
...
if (selectVisibleBookings) {
  selectVisibleBookings.addEventListener('change', function() {
    getVisibleCards().forEach(function(card) {
      const input = card.querySelector('.booking-check');

      if (input) {
        input.checked = selectVisibleBookings.checked;
      }
    });
